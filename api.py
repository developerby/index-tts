        ...
        try:
            if emo_vector is not None:
                ...
                if len(emo_vector) != 8:
                    ...
                    return
                emo_vector = self.tts.normalize_emo_vec(emo_vector, apply_bias=True)
            
            if emo_alpha is not None and not (0.0 <= emo_alpha <= 0.8):
                await websocket.send_json({"error": f"emo_alpha must be between 0.0 and 0.8, got {emo_alpha}"})
                return
